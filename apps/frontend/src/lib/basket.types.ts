...
// ---------------------------------------------------------------------------
// Response types — Finland reference total (task 4.8)
// ---------------------------------------------------------------------------

/**
 * One chosen basket line's Finland reference contribution — the product's
 * newest Alko observation with full provenance, mirroring the core-domain
 * contract. The per-line figures live here so the aggregate total traces
 * to exact reference rows.
 */
...
/** A chosen basket line with no Finland reference — absent from the total. */
...
/**
 * Finland reference total for the chosen basket (task 4.8).
 *
 * `available` sums ONLY the lines that carry a reference — `missingLines`
 * names the excluded remainder so a partial total never reads as
 * complete. `unavailable` states that no line has a reference at all;
 * the UI renders that as an explicit note, never a zero.
 */
...
export interface BasketOptimizationResult {
   ...
   * Optional here for tolerance: idempotency-cache HITs of results stored
   * before the field existed carry no key — the UI renders nothing for
   * it, the same render-nothing state the packing field uses.
   */
