...
describe('DeclarationGuidancePanel', () => {
  it('hides the panel and never fetches the declaration when the flag is off', async () => {
    ...
    const { container } = render(<DeclarationGuidancePanel recordId={55} />);
  ...
  it('renders derivation, deadline, checklist, caveats, sources, and the disclaimer', async () => {
    render(<DeclarationGuidancePanel recordId={55} />);
    ...
    expect(rates[0]).toHaveTextContent('Alcohol excise');
    expect(rates[0]).toHaveTextContent('€12.34');
    expect(rates[0]).toHaveTextContent('€0.5218 per litre of pure alcohol');
    expect(rates[0]).toHaveTextContent('Rule version 2025.1');
    ...
    expect(deadline).toHaveTextContent(
      'Advance notice to customs is required',
    );
    expect(deadline).toHaveTextContent('Due 2026-09-10');
    expect(deadline).toHaveTextContent('14 days from');
