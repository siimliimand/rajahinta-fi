...
export default function ScenarioControls({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setName('');
      setStatus(`Scenario "${trimmed}" saved.`);
      await refreshScenarios();
    } catch {
      setError('Saving the scenario failed. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleLoad = (
    ...
    event.target.value = '';
    setError(null);
    setStatus(`Scenario "${selected.name}" loaded — calculation re-run.`);
  ...
  return (
    ...
    >
      <h2 className="mb-3 text-sm font-semibold text-gray-700">Scenarios</h2>
      ...
      {!canSave && (
        <p className="mt-1.5 text-xs text-gray-400">
          Select a product to enable saving.
        </p>
      )}
