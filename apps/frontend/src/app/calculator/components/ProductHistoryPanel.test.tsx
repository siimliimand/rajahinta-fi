...
describe('ProductHistoryPanel', () => {
  ...
  it('treats an unreachable flag endpoint as disabled (no history fetch)', async () => {
    ...
    const { container } = render(<ProductHistoryPanel productId={42} />);
    await waitFor(() => expect(container.firstChild).toBeNull());
  ...
  it('fetches the product-wide daily 90-day series and renders the chart when the flag is on', async () => {
    render(<ProductHistoryPanel productId={42} />);
  ...
  it('states "Data available from <date>" when history is truncated by data availability', async () => {
    ...
    expect(
      await screen.findByText('Data available from 26.7.2026'),
    ).toBeInTheDocument();
  });
  ...
  it('shows the available-from notice in the empty state when no series exists yet', async () => {
    ...
    expect(
      screen.getByText('Data available from 1.8.2026'),
    ).toBeInTheDocument();
  });
  ...
  it('refetches with metric=landed-cost when the metric is toggled', async () => {
    ...
    expect(
      await screen.findByText('Landed cost · history'),
    ).toBeInTheDocument();
  });
  ...
  it('refetches with a merchant filter when a merchant is selected (result view)', async () => {
    ...
    render(<ProductHistoryPanel productId={42} showMerchantFilter />);
    ...
    expect(select).toHaveTextContent('All merchants');
  ...
  it('degrades a rate-limited failure to a neutral retry affordance that refetches', async () => {
    ...
    expect(
      screen.getByText('Historical data is temporarily unavailable.'),
    ).toBeInTheDocument();
