...
export default function HistoryChart({
      ...
      <Card data-testid="history-chart-empty">
        ...
        {earliestAvailableObservationDate && (
          <p className="mt-1 text-xs text-gray-400">
            {t('dataAvailableFrom', {
              date: formatFullDate(parseDateMs(earliestAvailableObservationDate)),
            })}
          </p>
        )}
      </Card>
  ...
  for (const s of plottable) {
    for (const p of s.points) {
      const tMs = parseDateMs(p.periodStart);
      if (tMs < d0) d0 = tMs;
      if (tMs > d1) d1 = tMs;
    }
  }
  ...
  const x = (tMs: number) => INNER_LEFT + ((tMs - d0) / (d1 - d0)) * INNER_WIDTH;
    ...
    <Card data-testid="history-chart">
      ...
      <p className="mt-0.5 text-xs text-gray-400">
        {t('bucketsLine', {
          ...
          from: formatFullDate(d0),
          to: formatFullDate(d1),
        })}
      </p>
      ...
      >
        ...
        {ticks.map((tMs, i) => (
          <text
            ...
            className="fill-gray-500 text-[10px]"
          >
            {includeYear ? formatFullDate(tMs) : formatShortDate(tMs)}
          </text>
        ))}
          ...
          return (
            <g key={`marker-${i}`} data-testid="tax-marker">
              ...
              <title>
                {t('markerTitle', {
                  merchant: m.merchant,
                  classification: t(`classification.${m.classification}`),
                  date: formatFullDate(m.dateMs),
                })}
              </title>
            </g>
          );
        ...
        {ordered.map((s, si) => {
          ...
          const name = s.merchant ?? t('allMerchants');
          const segments = buildSegments(s.points, stepMs);
          return (
            <g key={`${si}-${name}`} data-testid="series-group">
              <title>{t('seriesTitle', { name, metric: metricLabel })}</title>
              {segments.map((seg, gi) =>
                seg.length === 1 ? (
                  <circle
                    key={`seg-${gi}`}
        ...
        <ul className="sr-only">
          {markers.map((m, i) => (
            <li key={`marker-desc-${i}`}>
              ...
              {m.label ? t('markerSrLabel', { label: m.label }) : ''}
            </li>
          ))}
        </ul>
      )}
