...
/**
 * Unique, descriptive metadata for the trip route
 * (price-intelligence-roadmap task 2.3): break-even and allowance-fill
 * framing for a cross-border trip, distinct from the site-default and
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'TripPage' });
  return {
    title: t('metaTitle'),
...
/**
 * Trip feasibility page (price-intelligence-roadmap task 2.3, the D2
 ...
 * HTML. The two-mode calculation flow is the client view in
 * `trip-view.tsx`, unchanged in behavior.
 */
export default async function TripPage({ params }: TripPageProps) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section
        aria-labelledby="trip-how-heading"
        className="mb-8 rounded-lg border border-gray-200 bg-gray-50 p-5"
      >
        <h2 id="trip-how-heading" className="mb-2 text-base font-semibold text-gray-900">
          {t('howTitle')}
        </h2>
