...
/**
 * Summarise violations into a concise warning string.
 *
 * If all violations are for the same pattern, uses the singular form.
 * Otherwise shows the count.
 */
function summarise(violations: readonly ContentViolation[]): string {
  if (violations.length === 0) return '';

  const uniquePatterns = new Set(violations.map((v) => v.pattern));
  if (uniquePatterns.size === 1) {
    const v = violations[0];
    return `Promotional language detected: "${v.pattern}" (${LANGUAGE_LABEL[v.language] ?? v.language})`;
  }

  return `${violations.length} content-policy violations found`;
}
