...
const GOVERNANCE_LABEL: Record<PermissionStatus, string> = {
  GRANTED: 'Granted',
  PENDING: 'Pending',
  REVOKED: 'Revoked',
  EXPIRED: 'Expired',
    ...
    >
      ...
      <ul className="mt-1.5 space-y-1.5" data-testid="merchant-freshness-list">
        {visible.map((merchant) => {
          ...
          ).map((s) => `${STATUS_LABEL[s]} ${formatShare(score.statusShares[s])}`);
          ...
          return (
            ...
            >
              ...
              {' — '}
              {score.offerCount}{' '}
              {score.offerCount === 1 ? 'offer' : 'offers'}
              {shares.length > 0 && ` · ${shares.join(' · ')}`}
              {score.freshestObservedAt !== null && (
                <> · Freshest observation {formatTimestamp(score.freshestObservedAt)}</>
              )}
              {' · '}
              Governance status {GOVERNANCE_LABEL[score.governancePermissionStatus]}
              {' · '}
              Computed {formatTimestamp(score.computedAt)}
            </li>
          );
        })}
      </ul>
