...
describe('MerchantFreshnessSection', () => {
  ...
  it('renders the factual summary per merchant when the flag is on', async () => {
    ...
    for (const row of rows) {
      expect(row).toHaveTextContent('12 offers');
      expect(row).toHaveTextContent('Verified 75%');
      expect(row).toHaveTextContent('Estimated 25%');
      ...
      expect(row).toHaveTextContent('Freshest observation 25.8.2026');
      expect(row).toHaveTextContent('Governance status Granted');
      expect(row).toHaveTextContent('Computed 27.8.2026');
    }
  ...
  it('renders nothing when the merchant list is empty (even with the flag on)', async () => {
    const { container } = render(<MerchantFreshnessSection merchants={[]} />);
  ...
  it('handles a pending governance status without presenting the merchant as granted', async () => {
    ...
    render(<MerchantFreshnessSection merchants={['merchant-a']} />);

    const row = await screen.findByTestId('merchant-freshness-row');
    expect(row).toHaveTextContent('Governance status Pending');
    // No observations → no freshest-observation claim.
    expect(row).not.toHaveTextContent('Freshest observation');
  });
});
