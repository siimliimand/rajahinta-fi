...
export default function BasketComparisonSection() {
  ...
  const handleOptimize = useCallback(async () => {
    ...
    try {
      ...
      setOptimizeError(basketErrorMessage(kind));
    } finally {
      setOptimizing(false);
  ...
  // ── Destination countries (same list as BasketBuilder) ──
    ...
    <section className="mt-8 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">
          Compare how a multi-item basket&apos;s costs distribute across
          stores. All rankings are objective and neutral.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        ...
        {atCapacity ? (
          <p className="text-sm text-amber-700">
            Basket is full ({MAX_ITEMS} items maximum). Remove an item to add
            another.
          </p>
        ) : (
          <>
          ...
          <>
            {atCapacity && (
              <p className="mb-3 text-xs text-amber-600">
                Maximum {MAX_ITEMS} items reached. Remove an item to add
                another.
              </p>
            )}
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <fieldset>
          ...
          <div className="space-y-2">
            {(Object.entries(TRANSPORT_LABELS) as [TransportArrangement, string][]).map(
              ([value, label]) => (
                <label
                  key={value}
      ...
      <div>
        ...
        >
          {optimizing ? 'Optimoidaan…' : 'Compare basket across stores'}
        </button>
