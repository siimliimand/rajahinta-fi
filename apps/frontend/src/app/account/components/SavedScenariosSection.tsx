 ...
 *    scenario list request is never fired. A failed flag lookup also
 *    degrades to hidden.
...
import { fetchProductsByIds, getFeatureFlags, listScenarios } from '@/lib/api';
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-gray-600">
        Your saved calculator input sets. Loading one on the calculator
        re-runs the calculation against current data.
      </p>
      ...
      {scenarios.length === 0 ? (
        <div className="mt-6 rounded-md bg-gray-50 p-4 text-center text-sm text-gray-500">
          ...
          <p className="mt-1">
            Save calculator inputs under a name from the{' '}
            <Link
              href="/calculator"
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-gray-500">
                  {productNames[scenario.inputs.productId] ??
                    `Product #${scenario.inputs.productId}`}
                  {' · '}
                  {scenario.inputs.quantity}{' '}
                  {scenario.inputs.quantity === 1 ? 'unit' : 'units'}
                  {' · '}
