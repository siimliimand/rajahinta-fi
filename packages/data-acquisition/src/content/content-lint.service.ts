/**
 * Content lint service.
 *
 * Scans product names and descriptions for banned promotional/subjective
 * adjectives in Finnish, English, and Swedish.  Acts as a **linter**, not a
 * blocker — all violations are warnings that feed into the pipeline report
 * without rejecting data.
 *
 * @module ContentLintService
 */
...
/**
 * A single content-policy violation detected during linting.
 */
...
@Injectable()
export class ContentLintService {
  /**
   ...
   * ```
   */
  lintProductContent(name: string, description: string): LintResult {
    const violations: ContentViolation[] = [
      ...scanField(name, 'name'),
