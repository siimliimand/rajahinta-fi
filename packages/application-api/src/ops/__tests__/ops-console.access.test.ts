 ...
 *
 * The spec scenario that gates everything else: unauthenticated or
 * non-operator callers are denied BEFORE any operational data is returned.
 * Also exercises the OpsAuditTrailService read over the durable audit store.
 *
 * Guards run against a real controller instance so the metadata is exactly
 * what production sees — the sibling guard-regression convention from the
 * merchants suite.
 *
