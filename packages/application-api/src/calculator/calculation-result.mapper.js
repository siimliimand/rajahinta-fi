...
exports.mapCalculationRecordToResult = mapCalculationRecordToResult;
// ---------------------------------------------------------------------------
...
function isReliabilityStatus(value) {
    return (typeof value === 'string' &&
        RELIABILITY_STATUSES.includes(value));
}
function isCostCategory(value) {
    return (typeof value === 'string' &&
        COST_CATEGORIES.includes(value));
}
/**
...
function toItemizedCost(raw) {
    ...
    if (!isCostCategory(entry.category))
        return null;
    ...
    const reliability = isReliabilityStatus(entry.reliability)
        ? entry.reliability
        : 'UNAVAILABLE';
    const cents = typeof entry.cents === 'number' && Number.isFinite(entry.cents)
        ? entry.cents
        : 0;
...
function parseRetailOfferIds(raw) {
 ...
 */
function parseAlkoBenchmark(raw) {
    if (typeof raw !== 'object' || raw === null)
        return null;
    const entry = raw;
    if (entry.status !== 'available')
        return null;
    if (typeof entry.referencePriceCents !== 'number' ||
    ...
    if (!isReliabilityStatus(entry.reliabilityStatus))
        return null;
...
function mapCalculationRecordToResult(input) {
    ...
    if (input.containerVersionLabel !== null &&
    ...
    return {
        ...
        classification: {
            ...
            confidence: 'LOW',
            evidence: [],
            evidenceSummary: 'Transaction classification is not persisted with the calculation ' +
        ...
        metadata: {
            ...
            alcoholByVolume: product?.alcoholByVolume !== null && product?.alcoholByVolume !== undefined
...
//# sourceMappingURL=calculation-result.mapper.js.map