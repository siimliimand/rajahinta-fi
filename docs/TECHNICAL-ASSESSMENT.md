...
8. ~~A frontend feature-flag bootstrap that does not flash: flags are fetched client-side after hydration, so gated UI can appear late. Inline the flag states in the initial HTML payload.~~ Moot: the feature-flag system was removed entirely (2026-09-07, owner decision); every feature ships enabled.
...
4. Operator console: implemented at `/ops` (the `OPERATOR_CONSOLE` gate was later removed with the feature-flag system, 2026-09-07 — the console is unconditionally reachable and still behind OpsAccessGuard), covering governance grants, dataset confirmations including FX publish with cache invalidation, and the correction queue, with every action audited (task 12.1); the trust-and-reach moderation surface (report queue, blacklist publish/appeals, blog publish, newsletter notify) extends it (change `trust-and-reach-roadmap`).
...
10. Trust and reach features (change `trust-and-reach-roadmap`, seven features, all live):
    - **Shop blacklist**: evidence-gated reports (`POST /api/v1/reports`, order reference + correspondence summary required), operator-reviewed publication against a published standard (3+ independent confirmed non-delivery reports or confirmed invalid business registration), appeal path (`packages/core-domain/src/blacklist/`, ops console queue). Display-only `merchantWarnings` blocks join product detail, search, and compare responses; a compliance test proves the responses byte-identical with zero, one, and many warnings (`tests/compliance/warnings-additive-neutrality.test.ts`).
    - **Verified outcomes**: an authenticated user reports the actual total of a calculation within 60 days, one report per record per account; the public accuracy statistic (count, share within 5% of estimate, as-of date) is computed read-time and labeled user-reported everywhere, with sample size and an honest zero state (`packages/core-domain/src/outcomes/`, `GET /api/v1/accuracy`).
    - **Rate-change blog + newsletter**: FI + EN drafts auto-created at the manual rate-version confirmation (fail-open), human publish only, public PUBLISHED-only endpoints; newsletter with double opt-in stored separately from alert consent, one-click immediate unsubscribe, intent-log delivery through the email worker so a retried send cannot duplicate an email (`packages/core-domain/src/content/`, `apps/email-worker/src/templates.ts`).
    - **Tax-change alerts**: TAX_CHANGE kind in the existing alert system — no threshold, evaluated at rate-version confirmation using the landed-cost attribution of the version delta, same intent-log and 24-hour cooldown as price alerts (`apps/api-worker/src/cron/tax-change-alert-evaluation.ts`).
    - **Share permalinks + embed**: frozen snapshots under random 22-char public ids with a personal-data strip assertion, public read-only share page with the structural disclaimer and OG metadata, and an embeddable calculator that keeps the age gate, disclaimers, and API guards intact (`packages/core-domain/src/sharing/`).
    - **€/g value ranking**: public per-category listing ordered deterministically by ethanol €/g ascending with per-row reliability status; informational copy only, proven not to feed any other ordering or calculation input (`GET /api/v1/unitprice/ranking`, `tests/compliance/accuracy-unitprice-input-isolation.test.ts`).
    - **Allowance-fill planner**: a fill objective in the trip mode that assembles the best basket inside the traveller allowance effective on the travel date, with per-line contribution, headroom, and dataset-version provenance; ferry offers stay in their separate display-only block, compliance-tested byte-identical (`POST /api/v1/trip/fill`, `tests/compliance/trip-fill-ferry-neutrality.test.ts`).
11. Trip fill frontend mode on the trip page: allowance input, candidate selection, itemized result; the existing ferry-offer block untouched.
12. Ops console moderation surface: report review queue, blacklist publish/appeal resolution, blog post publication, and the newsletter notify-subscribers broadcast, all behind OpsAccessGuard and appended to the durable audit trail.
...
- **Outcome and share-snapshot sweeps not yet scheduled:** the 24-month retention cap for `calculationOutcomes` (`CALCULATION_OUTCOME_RETENTION_DAYS = 730` in `packages/data-platform/src/d1/schema.ts`) and the 12-month hygiene cap for `shareSnapshots` (`packages/core-domain/src/sharing/sharing.types.ts`) exist as constants, and outcomes are deliberately excluded from the calculation-record sweep's table list so the record sweep can never destroy them — but neither cap has its own scheduled sweep yet. Until one is wired, outcome and snapshot rows accumulate.
- **Rate-confirmation hooks await a production confirmation path:** the confirmation seam (`apps/api-worker/src/cron/rate-confirmation.ts`) fans out tax-change alert evaluation and blog-draft creation fail-open, but the ops tax approve/reject routes currently fail closed with 503 (the rate-review store has no D1 counterpart, migrate-to-cloudflare 2.5 debt). Tax-change alerts, blog drafts, and the newsletter broadcast therefore have no operator-triggering action in production until that store lands; the hooks are unit-tested and invoked as soon as the confirmation action is real.
- **Trust-surface data volume:** the accuracy statistic, merchant warnings, and the €/g ranking are read-time queries over small tables. No caching layer exists in front of `GET /api/v1/accuracy` or the ranking endpoint; revisit if traffic makes the D1 roundtrips visible.
