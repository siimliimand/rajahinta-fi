...
The trip mode answers "what is worth bringing back"; the allowance-fill planner answers "what exactly should I buy with my duty-free allowance". Given a traveller allowance budget, the basket optimizer's bounded search fills the best basket that fits inside the limit resolved from the versioned traveller-allowance datasets by effective travel date. Every result is explainable — dataset version, candidate offers, and search bounds ride the response — and the curated ferry offers stay in their separate display-only block.
