...
A moderated, evidence-gated list of fraudulent or seriously unreliable merchants. Accounts report merchants with order evidence, operators review and publish only against a published standard, and a published entry produces a display-only warning wherever that merchant's offers appear. The list protects users without touching the calculator: warnings are additive blocks and results, rankings, and totals stay byte-identical whether zero, one, or many warnings are present.
