...
Account erasure (`anonymizeAccount`) SHALL be implemented for the production D1 wiring, not only the in-memory fallback. Erasure SHALL irreversibly pseudonymize identifiers (retaining an anonymized skeleton only where referential integrity of calculation records requires it), cascade to saved baskets and linked personal data, and record an audit event.

#### Scenario: Erasure against production D1

- **WHEN** `anonymizeAccount` is invoked for an account stored in D1
