...
- [x] 1.1 Fix the registry credential — add `permissions: contents: read, packages: write` and switch the GHCR login password to `secrets.GITHUB_TOKEN` in both deploy workflows; keep `REGISTRY_TOKEN` PAT as documented fallback if the `ghcr.io/rajahinta/rajahinta` package ACL blocks the repo token; verify with a pushed image (design D1) <!-- reopened post-merge (ghcr.io/rajahinta owner did not exist, push denied not_found) and closed by PR 23: IMAGE_NAME moved to ghcr.io/siimliimand/rajahinta with matching base-manifest images; verified green with pushed image in run 32529902593 -->
