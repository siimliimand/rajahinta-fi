...
Persist a time series per canonical product and per merchant offer covering foreign retail price, transport cost, applicable tax rate, and resulting landed cost at each observation. Serve chart rendering from materialized aggregates, and make tax-driven price changes identifiable against the versioned tax-rule history. Implements Task 2A (T2.1 through T2.5) of the implementation plan.
