...
│   │       │   ├── longero.adapter.ts        # longero.fi WooCommerce Store API feed (third live merchant, change onboard-longero-merchant)
│   │       │   ├── woo-store.adapter.ts      # shared WooCommerce Store API page-walk (parameterized by merchantId; alks/longero/kippis adapters are thin subclasses)
│   │       │   ├── kippis.adapter.ts         # kippis.net WooCommerce Store API feed (fourth live merchant, change onboard-kippis-merchant)
...
| `packages/data-acquisition` | Merchant feed ingestion pipeline (Alko domestic reference; alks.fi, longero.fi, and kippis.net Store APIs), carrier sources (Posti), data-quality checks, rate-review scheduler | `adapters/`, `services/`, `__fixtures__/` |
...
| kippis.net (retailer, seller country FI) | Live — fourth merchant feed, `GRANTED` in staging and production (direct D1 by the operator during onboarding; `audit_events` deviation recorded, change `onboard-kippis-merchant`) | `packages/data-acquisition/src/adapters/kippis.adapter.ts` (change `onboard-kippis-merchant`): WooCommerce Store API (`https://www.kippis.net/wp-json/wc/store/v1/products`) through the shared page-walk (`woo-store.adapter.ts`), source type `RETAILER_API` (documented scraping rights), EUR minor units, daily cadence (86,400,000 ms) from the registry row; Finnish category vocabulary added to the mapper; SKU→EAN accepts prefixed, bare-13, and GTIN-14 forms; first ingest 636 products / 635 offers with 111 (17.5%) EAN-joined into the pre-existing catalog; read-only full-catalog audit via `scripts/kippis-catalog-sweep.ts` |
...
- **Merchant feeds for the further EUR markets** — with `longero.fi` (seller country EE) and `kippis.net` (seller country FI) live as the third and fourth merchants alongside `alks.fi` (DE), the remaining markets are the deliberate deferrals; the adapter registry and the governance gate onboard a new merchant without a deploy, and the currency union is already pinned to `'EUR'`
