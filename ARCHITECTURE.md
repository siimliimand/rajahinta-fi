...
| **Operator console** | OpsAccessGuard-protected: governance grants, dataset confirmations, correction queue, report review queue (link/reject), blacklist publish + appeal inbox, blog post publish, guide draft create/edit/publish (GUIDE kind, no rate-version provenance), newsletter notify-subscribers broadcast | `apps/frontend/src/app/[locale]/ops/`, `apps/api-worker/src/routes/ops.routes.ts` |
