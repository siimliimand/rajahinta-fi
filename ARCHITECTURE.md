...
│   │       │   ├── longero.adapter.ts        # longero.fi WooCommerce Store API feed (third live merchant, change onboard-longero-merchant; reuses alks.parser)
│   │       │   ├── posti-rate.source.ts      # Posti carrier rate source (governance-gated, fixture-pinned)
│   │       │   ├── fransberg-rate.source.ts  # Fransberg carrier rate source (manually curated dataset, monthly sync)
...
| `packages/data-acquisition` | Merchant feed ingestion pipeline (Alko domestic reference, alks.fi and longero.fi Store APIs), carrier sources (Posti live feed, Fransberg curated dataset), data-quality checks, rate-review scheduler | `adapters/`, `services/`, `__fixtures__/` |
...
| **Home page**        | Hero with a one-sentence value proposition and calculator CTA, static trust row (data sources, reliability model, methodology link), a static worked-example section after the hero (example-labeled Finland-vs-cross-border figures, no API call), and a FAQ section linking PUBLISHED guide entries (server-side fetch, degrades to nothing) | `apps/frontend/src/app/[locale]/page.tsx`                           |
...
| **Comparison page**  | Server shell with unique metadata over the side-by-side product comparison with sort controls; store-grouped multi-store comparison                          | `apps/frontend/src/app/[locale]/compare/`                          |
| **Basket page**      | Server shell with unique metadata over the basket builder (items, quantities, destination, transport arrangement) and optimization results with neutral cost-ordered alternatives; sticky desktop summary rendering the optimizer's `finlandReference` (Finland goods total + cheaper/dearer difference) | `apps/frontend/src/app/[locale]/basket/`                          |
...
| **Roadmap feature pages** | Live for all visitors: alerts management (account), event calculator with occasion templates, trip feasibility with route presets, the break-even card, fill mode and the visually distinct ferry-offer block, curated lists (`lists/[slug]`), what-if simulator labeled "Scenario calculator" (URL unchanged) with embeddable widget, group-order session page with a persistent settlement-happens-outside note — all tool pages served as server shells with unique metadata | `apps/frontend/src/app/[locale]/account/`, `.../event/`, `.../trip/`, `.../lists/`, `.../what-if/`, `.../group-order/` |
| **Trust-and-reach pages** | Live for all visitors: blog index and per-locale slug pages (PUBLISHED posts only, sitemap-included), newsletter subscribe form (blog + footer, explicit consent separate from price alerts) with confirmation landing page, public share page `/share/[publicId]` rendering a frozen snapshot with DisclaimerBanner and OG metadata, embeddable calculator `/embed/calculator` (minimal chrome, iframe-friendly headers, age gate + disclaimers intact), €/g value page `/value` with per-category ranking, status badges, and unique metadata, About and Contact pages (server shells, sitemap-included; Contact names the correction mechanism), register benefits copy, merchant warning badges on product/compare/search, accuracy statistic in the home trust row and on the methodology page (sample size + user-reported wording, honest zero state), report-outcome form on in-window account history records | `apps/frontend/src/app/[locale]/blog/`, `.../newsletter/`, `.../share/`, `.../embed/`, `.../value/`, `.../about/`, `.../contact/`, `.../products/`, `.../page.tsx` |
| **Insight pages** | Live for all visitors: `/savings` per-category gap listing (as-of + coverage counts in the header, reliability + confidence badges, ordering rule stated in the copy, honest zero state) plus the deterministic market-overview section (`GET /api/v1/savings/overview`: average observed price per category, largest observed cross-border difference with deterministic tie-break, observed product count), `/allowances` date-addressable explorer (verbatim citations as evidence links, version label + effective window, version history, guidance-not-legal-advice framing), `/guides` + `/guides/[slug]` (GUIDE-kind posts only, cross-links to /allowances and /trip), the product-page price-context line (delta vs the 90-day median with window + as-of, insufficient-history state renders no percentage), and the product-page price-history section (30/90/365-day chart with data-table alternative from the historical API) | `apps/frontend/src/app/[locale]/savings/`, `.../allowances/`, `.../guides/`, `.../products/`, `apps/api-worker/src/routes/savings.routes.ts` |
...
1. **Data Acquisition** → the hourly producer (each merchant gated by its registry `pollingIntervalMs` via epoch-aligned interval buckets — a merchant enqueues only on the tick after a boundary crossing, so daily rows fire once per 24 h and hourly rows behave unchanged) resolves the merchant's adapter from the `FEED_ADAPTERS` registry keyed by `merchantId`: `alko` fetches the domestic reference assortment JSON; `alks` walks the WooCommerce Store API sequentially (`per_page=100` capped by `X-WP-TotalPages`, page failures collected, never thrown), parses ABV/volume from the product name (a record whose ABV or volume cannot be parsed still ingests as ESTIMATED), carries an optional feed weight into `weight_grams` on the product master, and ignores image fields; `longero` mirrors the alks walk through the same parser unchanged (`longero.adapter.ts`, change onboard-longero-merchant — the registry row carries country `EE` because the operator TOVAGLUKE OÜ is Estonian despite the `.fi` domain). All three adapters map to `RawFeedRecord` (all prices are EUR — the currency union is pinned to the `'EUR'` literal, so there is no conversion step), pipeline orchestrator runs data-quality checks, `DataMappingService` normalizes fields, `UpsertPortAdapter` persists via `ProductRepository.upsertByEan()`.
...
| Fransberg carrier rates (fransberg.eu) | Source implemented (curated dataset)   | `packages/data-acquisition/src/adapters/fransberg-rate.source.ts`: home-delivery prices transcribed 2026-09-17 (DE/HR → FI, parcels by count over 31.5 kg brackets, pallets 720 kg); no live feed exists, so the monthly `fransberg-rate-refresh` cron syncs the DB with the in-repo dataset and skips unchanged data; governance gate still applies — carrier "fransberg" needs a GRANTED record |
...
| alks.fi (retailer, seller country DE)  | Live — second merchant feed, `GRANTED` (staging bootstrap grant 2026-09-11 under the owner's blanket-permission policy; production grant pending, `docs/ingestion-runbook.md`) | `packages/data-acquisition/src/adapters/alks.adapter.ts` + `alks.parser.ts` (change `alks-feed-and-import-vat`): WooCommerce Store API (`https://alks.fi/wp-json/wc/store/v1/products`), source type `RETAILER_API`, EUR minor units, daily cadence (86,400,000 ms) from the registry row (the hourly tick enqueues only after the interval boundary — `docs/ingestion-runbook.md` §0); sequential pagination (`per_page` 100, `X-WP-TotalPages` bound, page failures collected not thrown), SKU-prefix EAN validation (non-matching rows to the correction queue), ESTIMATED on unparsed ABV/volume, optional weight into the product master, image fields ignored; read-only full-catalog audit via `scripts/alks-catalog-sweep.ts` |
| longero.fi (retailer, seller country EE) | Live — third merchant feed, `GRANTED` in staging and production (direct D1 by the operator during onboarding, change `onboard-longero-merchant`) | `packages/data-acquisition/src/adapters/longero.adapter.ts` (change `onboard-longero-merchant`): thin mirror of the alks walk reusing `parseAlksStoreProducts` unchanged — WooCommerce Store API (`https://longero.fi/wp-json/wc/store/v1/products`), source type `RETAILER_API` (operator TOVAGLUKE OÜ, Estonia; documented scraping rights), EUR minor units, daily cadence (86,400,000 ms) from the registry row, `depositSystem` false; read-only full-catalog audit via `scripts/longero-catalog-sweep.ts` |
...
- **Merchant feeds for the further EUR markets** — with `longero.fi` (seller country EE) live as the third merchant alongside `alks.fi` (DE), the remaining markets are the deliberate deferrals; the adapter registry and the governance gate onboard a new merchant without a deploy, and the currency union is already pinned to `'EUR'`
